# Source: https://docs.novu.co/agents/custom-code-agent/connect-your-first-agent

# Tutorial: Connect your first agent
...
This page keeps each setup step to the minimum you need to get going. For the full reference on any step, follow the linked pages under [Set up your agent](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/overview).

## [What you're building](https://docs.novu.co/#what-youre-building)
...
## [Before you start](https://docs.novu.co/#before-you-start)
...
## [Set up the project](https://docs.novu.co/#set-up-the-project)
...
### [Create the agent and connect a provider](https://docs.novu.co/#create-the-agent-and-connect-a-provider)
...
For the full provider flow (Slack tokens, install, permissions), see the [Quickstart](https://docs.novu.co/agents/custom-code-agent/quickstart) and [Create an agent](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/create-an-agent).

### [Scaffold the bridge app](https://docs.novu.co/#scaffold-the-bridge-app)
...
Run it in the directory where you want the project. The CLI generates a Next.js app with a starter agent. For details, see [Scaffold your project](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/scaffold-your-project).

### [Run it locally](https://docs.novu.co/#run-it-locally)
...
## [Where the code goes](https://docs.novu.co/#where-the-code-goes)
...
## [Build the agent](https://docs.novu.co/#build-the-agent)

Follow the steps below to add handlers, cards, metadata, an LLM, and conversation resolution to `support-agent.tsx`. For the API reference behind each step, see [Handle events](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/handle-events), [Reply](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/reply), and [Signals](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/signals).

### [Define the agent shell](https://docs.novu.co/#define-the-agent-shell)
...
### [Handle the first message](https://docs.novu.co/#handle-the-first-message)
...
For all card components, see [Interactive cards](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/reply#interactive-cards).

### [Use metadata for context](https://docs.novu.co/#use-metadata-for-context)
...
Read it back with `ctx.metadata.get('topic')` on the next message. To alert on-call for technical issues, use `ctx.trigger`. For details, see [Trigger a workflow](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/signals#trigger-a-workflow).

### [Answer follow-ups with an LLM](https://docs.novu.co/#answer-follow-ups-with-an-llm)
...
- For files in replies, use `ctx.reply` with the `files` option. For details, see [Sending attachments](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/reply#sending-attachments).

### [Resolve the conversation](https://docs.novu.co/#resolve-the-conversation)
...
## [Try it out](https://docs.novu.co/#try-it-out)
...
Open the [conversation in the dashboard](https://docs.novu.co/agents/conversations) to see the messages, metadata, and the resolution summary recorded for the thread.

## [Complete agent](https://docs.novu.co/#complete-agent)
...
## [How the pieces fit together](https://docs.novu.co/#how-the-pieces-fit-together)
...
[**Give the model real context**\\ \\ Add RAG or tool calls so the model can query your API or docs.](https://sdk.vercel.ai/docs/foundations/tools) [**Capture reactions**\\ \\ Add `onReaction` for thumbs-up/down feedback.](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/handle-events#onreaction) [**Send a CSAT email**\\ \\ Use `ctx.trigger` after resolution for a follow-up survey workflow.](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/signals#trigger-a-workflow) [**Build richer cards**\\ \\ Dropdowns, links, text inputs, and multi-action cards.](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/reply#interactive-cards) [**Going to production**\\ \\ Run locally, deploy to development, and publish to production.](https://docs.novu.co/agents/custom-code-agent/going-to-production)

[Concepts\\ \\ Entities, lifecycle, and handler building blocks for a custom code agent with Novu.](https://docs.novu.co/agents/custom-code-agent/concepts) [Overview\\ \\ What you configure when setting up a Novu agent: dashboard settings, bridge, providers, and handler code.](https://docs.novu.co/agents/custom-code-agent/setup-your-agent/overview)

### On this page

[What you're building](https://docs.novu.co/#what-youre-building) [Before you start](https://docs.novu.co/#before-you-start) [Set up the project](https://docs.novu.co/#set-up-the-project) [Create the agent and connect a provider](https://docs.novu.co/#create-the-agent-and-connect-a-provider) [Scaffold the bridge app](https://docs.novu.co/#scaffold-the-bridge-app) [Run it locally](https://docs.novu.co/#run-it-locally) [Where the code goes](https://docs.novu.co/#where-the-code-goes) [Build the agent](https://docs.novu.co/#build-the-agent) [Define the agent shell](https://docs.novu.co/#define-the-agent-shell) [Handle the first message](https://docs.novu.co/#handle-the-first-message) [Use metadata for context](https://docs.novu.co/#use-metadata-for-context) [Answer follow-ups with an LLM](https://docs.novu.co/#answer-follow-ups-with-an-llm) [Resolve the conversation](https://docs.novu.co/#resolve-the-conversation) [Try it out](https://docs.novu.co/#try-it-out) [Complete agent](https://docs.novu.co/#complete-agent) [How the pieces fit together](https://docs.novu.co/#how-the-pieces-fit-together) [Next steps](https://docs.novu.co/#next-steps)

Copy page as markdown[Edit this page on GitHub](https://github.com/novuhq/docs/edit/main/content/docs/agents/custom-code-agent/connect-your-first-agent.mdx)Open in ChatGPTOpen in Claude